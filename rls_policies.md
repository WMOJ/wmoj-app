...
In practice these policies exist so that the sign-up flow can create the row. Once created, all meaningful admin actions are gated by the active-admin check on other tables (see below).
...
| **Managers can update problems** | An active manager can update *any* problem, regardless of who created it. They are not restricted to problems they created. There is no restriction on which fields they can change. |
...
| **Managers can update contests** | An active manager can update *any* contest, regardless of who created it, with no field restrictions. |
...
| **cp_update_own** | A logged-in user can update their own participation record (e.g. their score or status). They cannot update someone else's. |
...
- **Update any problem** — regardless of who created it, with no field restrictions
- **Update any contest** — regardless of who created it, with no field restrictions
- Read and manage their own `managers` row

Managers **cannot** create or delete problems or contests, and they have no special access to `admins`, `countdown_timers`, `join_history`, or `contest_participants` beyond what a regular user has.

### Admin (logged in, has an active row in `admins`)

Full control over the system:

- **Problems:** create (attributed to themselves), update their own, delete their own — but cannot change `is_active` on their own problems via the admin update policy
- **Contests:** create (attributed to themselves), update their own, delete their own — same `is_active` restriction applies
- **Contest participants:** read, create, update, and delete any record for any user
- **Countdown timers:** full access to every user's timers
- **Join history:** full access — read, insert, update, delete any record
- **Managers:** full control — can create, read, update, and delete any manager row (this is how manager accounts are provisioned and revoked)
...
2. **Admins own their own content.** An admin can only create/delete the problems and contests they personally created. There is no policy allowing one admin to delete another admin's content.

3. **Managers are editors, not owners.** Managers can update any problem or contest but cannot create or delete them. They are essentially a moderation/editing role.
...
5. **Submissions have no ownership enforcement at the DB level.** The insert policy allows any authenticated user to insert a row with `with_check: true`, meaning the database does not verify that the `user_id` field matches the inserting user. The application code is solely responsible for setting this correctly.

6. **No cross-admin control.** There is no policy that lets one admin manage another admin's row. Admin row management is self-service only (or done outside of RLS, e.g. via a service role).
